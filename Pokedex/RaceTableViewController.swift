...
class RaceTableViewController: UITableViewController {
        ...
        if segue.identifier == "showWebView" {
            if let indexPath = tableView.indexPathForSelectedRow {
                if let destVC = segue.destination as? WebViewController {
                    ...
                    destVC.race = type?.races[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = type!.races[indexPath.row].name
        cell.detailTextLabel?.text = String((type!.races[indexPath.row].code))
        cell.imageView?.image = UIImage(named: (type!.races[indexPath.row].icon))
        return cell
    }
